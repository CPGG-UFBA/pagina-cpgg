...
export function EditButtonCalendars({ onClick, isEditMode, onLogout }: EditButtonCalendarsProps) {
  useEffect(() => {
    const oldButton = document.getElementById('floating-edit-button-calendars')
    if (oldButton) oldButton.remove()

    const button = document.createElement('button')
    button.id = 'floating-edit-button-calendars'
    ...
    return () => {
      const btn = document.getElementById('floating-edit-button-calendars')
      if (btn) btn.remove()
    }
