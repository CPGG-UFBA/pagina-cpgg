...
export function EditButtonProjects({ onClick, isEditMode, onLogout }: EditButtonProjectsProps) {
  useEffect(() => {
    const oldButton = document.getElementById('floating-edit-button-projects')
    if (oldButton) oldButton.remove()

    const button = document.createElement('button')
    button.id = 'floating-edit-button-projects'
    ...
    return () => {
      const btn = document.getElementById('floating-edit-button-projects')
      if (btn) btn.remove()
    }
